	...
	void resolve(Contact contact)
	{
		...
		if (!math::is_nearly_zero(contact.penetration))
		{
			if (math::is_nearly_zero(total_inv_mass))
			{
				...
				const math::FVec2 impact_translation = contact.normal * contact.penetration;
		...
		{
			math::FVec2 collider_velocity = impact_velocity + contact.collision_pair.first->get_velocity();
		...
		{
			math::FVec2 collided_with_velocity = impact_velocity + contact.collision_pair.second->get_velocity();
