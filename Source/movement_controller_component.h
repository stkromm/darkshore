...
public:
	...
	{
		static std::shared_ptr<ComponentType> sprite_type = std::make_shared<ComponentType>(ComponentType{ "movement-controller" });
		return sprite_type;
	}
	...
	void key_press(const size_t k, const  size_t m, const  size_t s, const  size_t a)
	{
		...
		if (k == GLFW_KEY_W) {
			get_owner()->get_component<RigidBodyComponent>()->get_rigid_body()->add_impuls({ 0,300 });
		...
		if (k == GLFW_KEY_A) {
			get_owner()->get_component<RigidBodyComponent>()->get_rigid_body()->add_impuls({ -300,0 });
